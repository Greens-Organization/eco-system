...
Here's how to switch your eco-system project to use Hypertune for feature flags!
...
## 3. Update the `chaves.ts` file in the `feature-flags` package

Use the `NEXT_PUBLIC_HYPERTUNE_TOKEN` environment variable in the call to `createEnv`:

```ts packages/feature-flags/chaves.ts {6-8,14}
...
```sh Terminal
bun remove @pack/analytics --filter @pack/feature-flags
```

Then, install the new dependencies:

```sh Terminal
bun add hypertune server-only --filter @pack/feature-flags
```
...
```sh Terminal
bun build --filter @pack/feature-flags
```
...
const hypertuneSource = createSource({
  token: chaves().NEXT_PUBLIC_HYPERTUNE_TOKEN,
});
