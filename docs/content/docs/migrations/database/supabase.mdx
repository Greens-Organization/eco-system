...
Here's how to switch from Neon to [Supabase](https://supabase.com) for your `eco-system` project.
...
```sh Terminal
bun install -D supabase --filter @pack/db
```

## 5. Update the database package

Update the `db` package. We'll remove the Neon extensions and connect to Supabase directly, which should automatically use the environment variables we set earlier.
...
Now you can run the migration from the root of your `eco-system` project:
