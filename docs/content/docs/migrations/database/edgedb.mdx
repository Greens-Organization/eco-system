...
`eco-system` uses Neon as the database provider with Prisma as the ORM as well as Clerk for authentication. This guide will provide the steps you need to switch the database provider from Neon to EdgeDB.
...
Here's how to switch from Neon to [EdgeDB](https://edgedb.com) for your `eco-system` project.
...
## 2. Swap out the required dependencies in `@pack/db`

Uninstall the existing dependencies...

```sh Terminal
pnpm remove @prisma/adapter-neon @prisma/client prisma --filter @pack/db
```
...
## 3. Setup EdgeDB in `@pack/db` package

In the `@pack/db` directory, run:

```sh Terminal
bunx edgedb project init --server-instance <org_name>/<instance_name> --non-interactive
```
...
You can also delete `prisma/` directory from the `@pack/db`:

```sh Terminal
rm -fr packages/db/prisma
```
...
```sh Terminal
bunx migration create
bunx migration apply
```
...
import edgeql from '@pack/db/dbschema/edgeql-js';
