---
title: Testing
description: How we've configured testing in eco-system.
---

![/images/testing.png](/images/testing.png)

eco-system uses [Vitest](https://vitest.dev/) for unit testing. It's a fast and lightweight testing framework that's compatible with Jest's API. Unit tests are run automatically as part of the `build` task in Turborepo.
...
as well as adding the `@pack/testing` package to the `devDependencies` section of the `package.json` file:
...
export { default } from '@pack/testing';
```
