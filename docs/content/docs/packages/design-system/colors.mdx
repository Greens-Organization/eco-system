...
eco-system makes use of the CSS variables offered by [shadcn/ui](https://ui.shadcn.com/). They're a brilliant way of abstracting the scaling and maintenance difficulties associated with [Dark Mode](/packages/design-system/dark-mode) and whitelabelling.
...
You can also access colors in JavaScript through the `tailwind` utility exported from `@pack/tailwind-config`, like so:

```tsx title="component.tsx"
import { tailwind } from '@pack/tailwind-config';
