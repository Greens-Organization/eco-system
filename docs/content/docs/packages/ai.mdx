---
title: AI
description: How to use AI in eco-system.
---

![/images/ai.png](/images/ai.png)

eco-system has a pre-configured `ai` package that uses [ai-sdk](https://sdk.vercel.ai/) to provide a simple interface for interacting with AI models.

## Usage

To use AI functionality, you can import it from the `@pack/ai` package.
...
```ts generate.ts
import { generateText } from '@pack/ai';
import { provider } from '@pack/ai/lib/provider';
import { models } from '@pack/ai/lib/models';
...
```tsx chatbot.tsx
import { Thread } from '@pack/ai/components/thread';
import { Message } from '@pack/ai/components/message';
...
You can use the `paymentsAgentToolkit` to add payments capabilities to your AI agent for financial services. As eco-system uses Stripe by default, here's an example of how to use it:

```ts generate.ts
import { generateText } from '@pack/ai';
import { paymentsAgentToolkit } from '@pack/payments/ai';
import { provider } from '@pack/ai/lib/provider';
import { models } from '@pack/ai/lib/models';
...
If you want to use a different provider, you can modify the `provider` in `@pack/ai/lib/provider.ts` to use the one you want. For example, to use Anthropic, you can change it to:
