...
eco-system uses [Nosecone](https://docs.arcjet.com/nosecone/quick-start) to set HTTP response headers related to security.
...
Recommended headers are set by default and configured in `@pack/security/middleware`. Changing the configuration here will affect all apps.
...
The CSP header is not set by default because it requires specific configuration based on the eco-system features you have enabled.
